...
var warningNoLastAppliedConfigAnnotation = "Warning: %[1]s apply should be used on resource created by either %[1]s create --save-config or %[1]s apply\n"
...
func Apply(context string, namespace string, in io.Reader, out, errOut io.Writer, options *ApplyOptions) error {
	...
	config := clientcmd.NewNonInteractiveClientConfig(*loadedConfig, context, &clientcmd.ConfigOverrides{}, loader)
	f := cmdutil.NewFactory(config) //clientcmd.NewDefaultPathOptions())
	...
	includeUninitialized := true //cmdutil.ShouldIncludeUninitialized(cmd, options.Prune)
	r := f.NewBuilder().
		Unstructured().
		Schema(schema).
		ContinueOnError().
	...
	if options.Prune {
		options.PruneResources, err = parsePruneResources(r.Mapper().RESTMapper, []string{})
		if err != nil {
			return err
		}
	}

	dryRun := false //cmdutil.GetFlagBool(cmd, "dry-run")
	...
	err = r.Visit(func(info *resource.Info, err error) error {
		...
		if !dryRun {
			...
			overwrite := true //cmdutil.GetFlagBool(cmd, "overwrite")
